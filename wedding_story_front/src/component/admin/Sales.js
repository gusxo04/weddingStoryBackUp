...
const CompanyItem = (props) => {
	...
	const sales = company.salesList.length > 0 && company.salesList[0] ? company.salesList[0].sales : "없음";
	...
	return (
		<tr>
			...
			<td>{salesValue ? salesValue : "없음"}</td>
			<td>{salesValue ? Math.ceil(salesValue * 0.3 * 100) / 100 : "없음"}</td>
			<td>{salesValue ? Math.ceil(salesValue * 0.7 * 100) / 100 : "없음"}</td>
		</tr>
	);
};
