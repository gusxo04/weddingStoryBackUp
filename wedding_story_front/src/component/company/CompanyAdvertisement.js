...
const CompanyAdvertisement = () => {
    ...
    useEffect(()=>{
        axios.get(`${backServer}/company/advertisement/${companyNo}/${reqPage}`).then((res)=>{
            console.log(res);
    ...
    return(
        ...
        <div className="list-wrap">
          <div className="list-title">광고 목록<span className="span" ref={spanRef} style={{marginLeft : "20px" ,display : "none"}}>* 광고요청은 최대 5개 까지만 가능합니다.</span></div>
 
          <div className ="navi-request">
            <Link to={"/company/advertisementRequest"}><button type="button" ref={insertRef} disabled={disableButton} >광고 신청</button></Link>
          </div>
...
const AdvertItem = (props) =>{
    ...
    return(
        <tr>
            ...
            <td style={{ width: "10%" }}>{advert.adRank}</td>
