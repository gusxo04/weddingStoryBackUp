...
import { companyNoState, loginIdState, loginNoState, memberCodeState, memberTypeState } from "../utils/RecoilData";
			...
			<table style={{ width: "97%" }}>
				...
				<tbody>
					<tr>
						...
						<td style={{ width: "33%" }}>
							<input name="memberName" value={`${member.memberName}`} onChange={changeInput}></input>
						</td>
						<th style={{ width: "5%" }}></th>
						<th style={{ width: "14%" }}>전화번호</th>
						<td style={{ width: "33%" }}>
							<input name="memberPhone" value={`${member.memberPhone}`} onChange={changeInput} onBlur={checkPhone}></input>
					...
					<tr>
						<th>아이디</th>
						<td>
							<input name="memberName" value={`${member.memberId}`} readOnly />
						</td>
						<th></th>
						<th>비밀번호</th>
					...
					<tr>
						<th>회원 성별</th>
						<td>
							<input name="memberName" value={`${member.memberGender}`} readOnly />
						</td>
						<th></th>
						<th>이메일 주소</th>
						<td>
							<input name="memberName" value={`${member.memberEmail}`} readOnly />
						</td>
					</tr>
				</tbody>
				{`${member.memberType}` === "1" ? (
					<>
						...
						<tbody>
							<tr>
								<th>배우자 성함</th>
								<td>
									{`${member.partnerName}` !== "null" ? (
										<input name="partnerName" value={`${member.partnerName}`} onChange={changeInput}></input>
									) : (
										<input name="partnerName" value={"정보가 없습니다."} onChange={changeInput} readOnly></input>
									)}
								</td>
								<th></th>
								<th>배우자 아이디</th>
								<td>
									{`${member.partnerId}` !== "null" ? (
										<input name="partnerId" value={`${member.partnerId}`} onChange={changeInput}></input>
									) : (
										<input name="partnerId" value={"정보가 없습니다."} onChange={changeInput}></input>
									)}
								</td>
							</tr>
						</tbody>
				...
				<tbody className="mypage-info-btnBox">
					<tr>
						<th colSpan={5}>
							<div>
								<button className="mypage-info-modifybtn" onClick={modifyMember}>
									정보수정
								</button>
